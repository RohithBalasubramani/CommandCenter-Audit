...
class AgentBrain {
    ...
    scenarioMaxActions = 30;
    // SOTA exploration controls
    actionSignatures = [];
    targetUseCount = new Map();
    targetFailureCount = new Map();
    ...
    async decideAction(observation) {
        this.stepCount++;
        ...
        if (this.config.useVision && observation.screenshot && this.config.screenshotDir) {
            screenshotPath = path.join(this.config.screenshotDir, `step-${String(this.stepCount).padStart(3, '0')}.png`);
    ...
    recordResult(action, result, duration) {
        const entry = {
            step: this.stepCount,
        ...
        if (result.success) {
            if (action.progress)
                this.markOutcomesFromText(action.progress);
            if (action.reasoning)
                this.markOutcomesFromText(action.reasoning);
    ...
    categorizeFailure(error, actionLog) {
        if (!error && !actionLog?.length)
            return 'unknown';
        ...
        // Element not found
        const notFoundCount = lastActions.filter(a => a.error?.includes('not found') || a.error?.includes('no element') || a.error?.includes('No valid selector')).length;
        if (notFoundCount >= 2)
            return 'element_not_found';
        // Element not interactable
        const notInteractableCount = lastActions.filter(a => a.error?.includes('not interactable') || a.error?.includes('disabled') || a.error?.includes('covered')).length;
        if (notInteractableCount >= 2)
        ...
        // Form error
        const formErrorCount = lastActions.filter(a => a.action.type === 'type' && a.result === 'failed').length;
        if (formErrorCount >= 1)
            return 'form_error';
        // Assertion failures
    ...
    getPerceptionLog() { return this.perceptionLog; }
...
RECENT ACTIONS:
${this.actionHistory.slice(-5).map(a => `  Step ${a.step}: ${a.action.type} "${a.action.target?.name || ''}" → ${a.result}${a.error ? ` (${a.error.slice(0, 60)})` : ''}`).join('\n')}
...
${this.config.useVision ? '- Screenshot paths are provided — view them to see the actual screen.' : ''}`;
    ...
    buildObservationText(obs, screenshotPath, stuckSignals, replanPrompt) {
        const parts = [
        ...
        parts.push(`Elements (${obs.interactiveElements.length}):`, ...elements.map((el, i) => `  ${i + 1}. [${el.role}] "${el.name}"${el.disabled ? ' (disabled)' : ''}${el.value ? ` val="${el.value}"` : ''}${el.x !== undefined ? ` @${el.x},${el.y}` : ''}`));
        ...
        if (obs.toasts.length)
            parts.push(`Messages on screen: ${obs.toasts.join(' | ')}`);
        if (obs.errors.length)
            parts.push(`Error messages: ${obs.errors.join(' | ')}`);
        if (obs.consoleErrors?.length) {
        ...
        if (remaining.length)
            parts.push(`  Still need: ${remaining.join(', ')}`);
    ...
    parseAction(llmResponse) {
        ...
        try {
            ...
            if (parsed.progress)
                this.markOutcomesFromText(String(parsed.progress));
            if (parsed.reasoning)
                this.markOutcomesFromText(String(parsed.reasoning));
            return parsed;
        }
        catch (err) {
    ...
    // ─── Element Shuffling ──────────────────────────────────────────────
    async decideWithRetries(prompt, observation) {
    ...
    async tryConsensusSampling(prompt, observation) {
    ...
    scoreCandidate(action, observation) {
        ...
        if (key) {
            ...
            if ((this.targetUseCount.get(key) || 0) === 0)
                score += 0.5;
        }
        if (action.target) {
    ...
    normalizeAction(action, observation) {
        const normalized = {
            ...action,
    ...
    enforceDoneGate(action, observation) {
        this.markOutcomesFromObservation(observation);
        const remaining = this.getRemainingOutcomes();
        if (remaining.length === 0)
            return action;
        const nearBudget = this.stepCount >= Math.max(5, Math.floor(this.scenarioMaxActions * 0.85));
        const isStuckIntent = /stuck|blocked|cannot|can't|give up|no progress/i.test(`${action.reasoning} ${action.blocker || ''}`);
    ...
    shouldDiversify(action) {
        if (action.type === 'done')
            return false;
        if (this.ledger.stuckScore < 3)
            return false;
        const signature = this.buildActionSignature(action);
        return this.actionSignatures.slice(-6).filter(s => s === signature).length >= 2;
    }
    buildDiversifiedAction(observation, original) {
    ...
    markOutcomesFromObservation(observation) {
        const corpus = [
            observation.url,
            observation.title,
            observation.heading || '',
    ...
    extractKeywords(text) {
        return Array.from(new Set(this.normalizeText(text)
            .split(' ')
    ...
    guessEvidenceKeyword(remainingOutcomes, observation) {
        const source = [
            ...remainingOutcomes,
            observation.heading || '',
            ...observation.toasts,
    ...
    isTargetPresent(target, observation) {
    ...
    // ─── Action Caching ─────────────────────────────────────────────────
    getCachedHint(scenarioId) {
        const cached = this.actionCache.find(c => c.scenarioId === scenarioId && c.success);
        if (!cached)
            return '';
        const steps = cached.actions.slice(0, 5).map((a, i) => `  ${i + 1}. ${a.type} ${a.target?.name ? `"${a.target.name}"` : ''}${a.value ? ` = "${a.value}"` : ''}`).join('\n');
    ...
    // ─── Cross-Session Learning ─────────────────────────────────────────
    getLessonsForScenario(scenarioId) {
    ...
    extractLesson(failedActions, confusedSteps) {
        const parts = [];
