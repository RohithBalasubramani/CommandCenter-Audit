...
import { EVAL_WEIGHTS, EXPECTED_SCENARIOS, type RLConfig } from './rl-config.js'
...
export class RLEvaluator {
  ...
  async evaluateResponse(
    ...
    const overallScore =
      widgetCountMatch ? 1.0 * EVAL_WEIGHTS.widgetCountMatch : 0 +
    ...
    const reasoning = this.buildReasoning(
      ...
      responseQuality, latencyScore, normalizedScore, rating,
    )
  ...
  private checkScenarioRelevance(
    ...
    if (widgets.length === 0) return 0.5 // No widgets is neutral
  ...
  private suggestCorrection(
    ...
    const widgets = result.layout_json?.widgets || []
    if (widgets.length === 0) return 'No widgets returned for this query'
    ...
    if (
      (queryLower.includes('alert') || queryLower.includes('alarm') || queryLower.includes('warning')) &&
